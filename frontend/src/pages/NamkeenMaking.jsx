...
export default function NamkeenMaking() {
    const [product, setProducts] = useState([]);
    const navigate = useNavigate()
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/product?category=NamkeenMaking`)
            ...
            <Container>
                <section className="px-4 md:px-20 mb-10">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-sm font-semibold">Namkeen Making Machines</h3>
